...
```{r data}
# data <- here("data/AUTOMATIC_DATA_2021-02-24_1216.csv")
today <- params$today
data <- params$data
site <- params$site
dat <- data %>%
  ...
  filter(!(!is.na(date_vaccine_due) & date_vaccine_due <= as.Date("2021-01-10")))
...
kable(tab, caption = "Site summary", booktabs = TRUE, escape = FALSE) %>%
  kable_styling()
```
